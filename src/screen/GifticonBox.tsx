...
import { Text, Image, Box, Pressable, useColorModeValue, Center, ScrollView, VStack, HStack, Divider } from "native-base";
import { TabView, TabBar, SceneMap } from "react-native-tab-view";

function ownGiftcons() {
  ...
  const InventoryItem = ({
    ...
    <TouchableOpacity style={{opacity: isUsed ? 0.5 : 1}} onPress={onItemClick}>
...
function purchaseHistories() {
  ...
  const InventoryItem = ({
    ...
    <TouchableOpacity style={{opacity: isRefunded ? 0.5 : 1}} onPress={onItemClick}>
    <HStack space={3} alignItems='center'>
      <VStack>
        <Text fontWeight='bold'>{purchaseDate}</Text>
        <Text fontSize='sm'>
          {purchaseItems[0].giftName + ((purchaseItems.length > 0) && ` 외 ${purchaseItems.length - 1}개`)}
        </Text>
      </VStack>
    </HStack>
  </TouchableOpacity>
